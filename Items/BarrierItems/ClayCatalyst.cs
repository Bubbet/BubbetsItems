...
namespace BubbetsItems.Items.BarrierItems
{
    public class ClayCatalyst : ItemBase
    {
        ...
        public static BuffDef? BuffDef =>
            _buffDef ??= BubbetsItemsPlugin.ContentPack.buffDefs.Find("BuffDefClayCatalyst");
        ...
        protected override void MakeTokens()
        {
            ...
            AddToken("CLAYCATALYST_DESC",
                "Release a " + "{0}m barrier effect ".Style(StyleEnum.Heal) + "during the Teleporter event, " +
                "multiplying barrier decay ".Style(StyleEnum.Heal) + "on all nearby allies for " +
                "{1:0%}".Style(StyleEnum.Heal) + ".");
            AddToken("CLAYCATALYST_DESC_SIMPLE",
                "Releases a " + "13m ".Style(StyleEnum.Heal) + "(+3m per stack) ".Style(StyleEnum.Stack) +
                "barrier effect ".Style(StyleEnum.Heal) + "for all allies in teleporter events and holdout zones, " +
                "multiplying barrier decay ".Style(StyleEnum.Heal) + "by " + "80%".Style(StyleEnum.Heal) +
                " (-11% per stack)".Style(StyleEnum.Stack) + ".");
            SimpleDescriptionToken = "CLAYCATALYST_DESC_SIMPLE";
            AddToken("CLAYCATALYST_PICKUP",
                "Slow down barrier decay nearby the Teleporter event and 'Holdout Zones' such as the Void Fields.");
        ...
        public static void FixBarrier(CharacterBody __instance, RecalculateStatsAPI.StatHookEventArgs args)
        {
            ...
            args.barrierDecayMult -= 1 - instance.ScalingInfos[1].ScalingFunction(amount);
        }
            ...
            for (TeamIndex teamIndex = TeamIndex.Neutral; teamIndex < TeamIndex.Count; teamIndex += 1)
            {
                ...
                if (NetworkServer.active)
                {
                    ...
                    if (AnyPlayers != ptr)
                    {
                        if (AnyPlayers)
                        {
                            ptr = GameObject.Instantiate(ZoneObject,
                                __instance.healingNovaRoot ? __instance.healingNovaRoot : __instance.transform);
        ...
        public static GameObject ZoneObject
        {
            get
            {
                if (_zoneObject is null)
                {
                    _zoneObject = BubbetsItemsPlugin.AssetBundle.LoadAsset<GameObject>("ClayCatalystTeleporter");
                    _zoneObject.GetComponent<BuffWard>().buffDef = BuffDef;
                }
