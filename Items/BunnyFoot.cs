...
namespace BubbetsItems.Items
{
    public class BunnyFoot : ItemBase
    {
        ...
        protected override void MakeTokens()
        {
            ...
            AddToken("BUNNYFOOT_DESC_SIMPLE",
                "Gain the ability to bunny hop, increasing air control by " + "150% ".Style(StyleEnum.Utility) +
                "(+150% per stack) ".Style(StyleEnum.Stack) + "and jump control by " + "50% ".Style(StyleEnum.Utility) +
                "(+50% per stack)".Style(StyleEnum.Stack) + ".");
            SimpleDescriptionToken = "BUNNYFOOT_DESC_SIMPLE";
            AddToken("BUNNYFOOT_PICKUP", "Your little feets start quivering.");
            AddToken("BUNNYFOOT_LORE",
                "haha source go brrrr\n\n\n\n\n\nIf you complain about this item being bad you're just outing yourself as bad at videogames.");
        }
        ...
        protected override void MakeConfigs()
        {
            ...
            AddAirControl = sharedInfo.ConfigFile.Bind("General", "Bunny Foot Add Air Control", false,
                "Add a bit of vanilla air control when at low speeds with bunny foot.");
            IsLunar = sharedInfo.ConfigFile.Bind("General", "Bunny Foot Is Lunar", false,
                "Makes bunny foot a lunar item.");
        ...
        [HarmonyILManipulator,
         HarmonyPatch(typeof(ProjectileGrappleController.GripState),
             nameof(ProjectileGrappleController.GripState.FixedUpdateBehavior))]
        public static void FixGrapple(ILContext il)
        {
            ...
            c.EmitDelegate<Func<bool, ProjectileGrappleController.GripState, bool>>((b, grip) =>
            {
                if (!TryGetInstance(out BunnyFoot inst)) return b;

                return (!grip.owner.characterBody ||
                        grip.owner.characterBody.inventory.GetItemCount(inst.ItemDef) <= 0) &&
                       b;
            });
        }

        [HarmonyILManipulator, HarmonyPatch(typeof(Assaulter2), nameof(Assaulter2.OnEnter)),
         HarmonyPatch(typeof(Assaulter2), nameof(Assaulter2.OnExit)),
         HarmonyPatch(typeof(FocusedAssaultDash), nameof(FocusedAssaultDash.OnExit)),
         HarmonyPatch(typeof(EvisDash),
             nameof(EvisDash.OnExit))] //, HarmonyPatch(typeof(WhirlwindBase), nameof(WhirlwindBase.FixedUpdate))]
        ...
        public static void FixAssulter2Dash(ILContext il, MethodBase __originalMethod)
        {
            ...
            {
                if (!TryGetInstance(out BunnyFoot inst)) return vector3;
                var count = assaulter2.characterBody.inventory.GetItemCount(inst.ItemDef);
                if (count <= 0) return vector3;
                if (__originalMethod.Name != "OnExit") return ((IPhysMotor)assaulter2.characterMotor).velocity;
                var outputVelocity =
                    (Vector3)(__originalMethod.DeclaringType?.GetField("dashVector", (BindingFlags)(-1))
                        ?.GetValue(assaulter2) ?? Vector3.zero) *
                    (float)(__originalMethod.DeclaringType?.GetField("speedCoefficient")?.GetValue(assaulter2) ?? 0f) *
                    assaulter2.moveSpeedStat * inst.ScalingInfos[5].ScalingFunction(count);
                var inputVelocity = ((IPhysMotor)assaulter2.characterMotor).velocity;
                return outputVelocity.normalized *
                       Mathf.Sqrt(Mathf.Max(inputVelocity.sqrMagnitude, outputVelocity.sqrMagnitude));
            });
        }
        ...
        public static void FixMercDash(ILContext il)
        {
            ...
            c.GotoPrev(MoveType.After,
                x => x.MatchCallOrCallvirt<EntityState>("get_" + nameof(EntityState.characterMotor)));
            c.Emit(OpCodes.Dup);
            ...
            {
                if (!TryGetInstance(out BunnyFoot inst)) return input;
                if (motor.body.inventory.GetItemCount(inst.ItemDef) <= 0) return input;
        ...
        [HarmonyILManipulator,
         HarmonyPatch(typeof(GenericCharacterMain), nameof(GenericCharacterMain.ApplyJumpVelocity))]
        ...
        public static Vector3 DoJumpFix(Vector3 vector, CharacterBody characterBody)
        {
            ...
            if (!TryGetInstance(out BunnyFoot bunnyFoot)) return vector;
            ...
            return Time.time - bh.hitGroundTime > bunnyFoot!.ScalingInfos[1]
                .ScalingFunction(characterBody.inventory.GetItemCount(bunnyFoot.ItemDef))
                ? vector
                : addvel;
        }
            ...
            CharacterMotor motor)
        {
            ...
            if (inv) count = inv.GetItemCountEffective(bunnyFoot.ItemDef);
        ...
        public static Vector3 Accelerate(Vector3 velocity, Vector3 wishDir, float wishSpeed, float speedLimit,
            float acceleration, float deltaTime, Vector3 target, float num)
        {
            ...
            if (addspeed <= 0)
                if (!AddAirControl.Value)
                    ...
                    return target.sqrMagnitude < velocity.sqrMagnitude
                        ? velocity
                        : Vector3.MoveTowards(velocity, target, num * deltaTime); //return velocity;
